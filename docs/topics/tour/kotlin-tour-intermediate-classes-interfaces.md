...
If you want to create a class similar to the `PenTool` class which has the same behavior **except** for the value of the
`color` property, you can. First, create a new class that expects an object that implements the `DrawingTool` interface as a parameter,
like an instance of the `PenTool` class. Then, inside the class, override the `color` property.
...
code in the `CanvasSession` class can be very large. However, there is an alternative.

In Kotlin, you can use delegation to delegate the interface implementation to an instance of a class. To do this, use 
the `by` keyword. For example:

```kotlin
class CanvasSession(val tool: DrawingTool) : DrawingTool by tool
```

Here, `tool` is the name of the instance of the `PenTool` class that the implementations of member functions are delegated to.
